    ...
    public String getBlockerText(CTTask task, long currentMillis, boolean big) {
        if (task.isReady()) {
            if (task.isBlocked(currentMillis)) {
                return messages.getMessage(big ? STR_BLOCKED_BIG : STR_BLOCKED_SMALL, CTTimeUtil.formatMMSS(CTTimeUtil.timeRemainsTo(currentMillis, task.getBlockEnd(currentMillis))));
            }
