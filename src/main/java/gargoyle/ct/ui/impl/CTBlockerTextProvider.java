...
public class CTBlockerTextProvider {
    ...
    public String getBlockerText(CTTask task, long currentMillis) {
        if (task.isReady()) {
            ...
            if (task.isWarn(currentMillis)) {
                return CTTimeUtil.isInPeriod(TimeUnit.SECONDS, currentMillis, PERIOD, DELAY) ? messages.getMessage(STR_WARN, CTTimeUtil.formatMMSS(CTTimeUtil.timeRemainsTo(currentMillis, task.getBlockStart(currentMillis)))) : null;
            }
        }
        return null;
    }
