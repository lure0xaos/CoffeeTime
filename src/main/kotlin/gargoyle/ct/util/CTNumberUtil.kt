...
object CTNumberUtil {
    ...
    fun <T : Number> fromInt(type: KClass<T>, value: Int): T {
        return when (type) {
            ...
            else -> type.functions.first { f -> f.name == METHOD_VALUE_OF }.call(value.toString()) as T
        }
    }
    ...
    fun toRange(value: Int, min: Int, max: Int): Int {
        return min.coerceAtLeast(max.coerceAtMost(value))
    }

    fun <T : Number> toRange(min: T, max: T, value: T): Int {
        return toInt(min).coerceAtLeast(toInt(max).coerceAtMost(toInt(value)))
    }

    fun <T : Number> toInt(value: T): Int {
        return value.toString().toInt()
    }
}
