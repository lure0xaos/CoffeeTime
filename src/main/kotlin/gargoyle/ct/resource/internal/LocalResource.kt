...
open class LocalResource protected constructor(url: URL) : VirtualResource<LocalResource>(url.toExternalForm()) {
    ...
    private class LocalLocation(private val url: URL, val readPriority: Int, val writePriority: Int) {
        ...
        override fun equals(other: Any?): Boolean {
            ...
            return url.toExternalForm() == that.url.toExternalForm()
        }
    }
    ...
    private class LocalLocationReadPriority : Comparator<LocalLocation> {
        ...
        companion object {
            @Serial
            private val serialVersionUID = 3293380242047042026L
        }
    }

    private class LocalLocationWritePriority : Comparator<LocalLocation> {
        override fun compare(o1: LocalLocation, o2: LocalLocation): Int {
            return o1.writePriority.compareTo(o2.writePriority)
        }

        companion object {
            @Serial
            private val serialVersionUID = 1771423086301596535L
        }
    }
    ...
    companion object {
        ...
        private val locations: Array<LocalLocation>
            get() = try {
                arrayOf(
                    ...
                    LocalLocation(
                        File(System.getProperty(ENV_USER_DIR, DOT)).toURI().toURL(),
                        0,
                        0
                    ),
                    ...
                    LocalLocation(
                        URL(
                            LocalLocation::class.java.getResource("/run.bat")!!.toExternalForm()
                                .substringBeforeLast('/')
                        ), 0, 0
                    )
                )
        ...
        val homeDirectoryLocation: URL
            get() = try {
                File(System.getProperty(ENV_USER_HOME, DOT)).toURI().toURL()
